...
"""
Analytics Service for Blueplane Telemetry Core.
...
class AnalyticsService:
    """
    Analytics service that processes SQLite traces into DuckDB analytics.
    ...
    async def start(self) -> None:
        ...
        if not self.enabled:
            logger.info("Analytics service is disabled, not starting")
            return
        ...
        logger.info(f"Starting analytics service (interval={self.processing_interval}s, batch_size={self.batch_size})")
    ...
    async def run(self) -> None:
        ...
        while self.running:
            try:
                ...
                logger.info("Analytics service cancelled")
                break
